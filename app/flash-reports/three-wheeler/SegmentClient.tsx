...
export default function ThreeWheelerPage() {
  ...
  // Free-user detection — collapse forecast window for free users so their
...
const [segmentTextError, setSegmentTextError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadOemData() {
      try {
        ...
        console.error(err);
        if (!cancelled) {
          setOemError("Failed to load three-wheeler OEM segment share data");
          setOemRaw([]);
        }
      } finally {
        if (!cancelled) {
  ...
  () =>
    buildLeadershipGrowthSummary({
      ...
      emptyMessage: "No three-wheeler OEM segment share data available for the selected period.",
...
{altFuelHeaderLabel}
  ...
  >
    {oemError ? (
      ...
      <div className="h-[350px] flex items-center justify-center text-sm text-muted-foreground">
        Loading three-wheeler OEM segment share…
      </div>
  ...
  Note: Includes petrol, diesel, CNG, electric (EV), and other alternative-fuel vehicles.
         ...
         {showEvChartSection && (
  <ChartWrapper
    title="Three-Wheeler EV Electric OEM Segment Share"
