...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        ...
        if (!cancelled) setGraphId(cfg?.pv ?? null);
      } catch (err) {
  ...
  const yoyPV = pickSeries(
    prevYearBaseData ? { data: prevYearBaseData } : prevYearPoint,
    ["PV", "pv", "Passenger", "Passenger Vehicles"],
  );
  const growthSummary = formatGrowthWithYoY(latestPV, prevPV, yoyPV);
    ...
    <div className="min-h-screen py-0">
      <div className="mx-auto w-[95vw] xl:w-[93vw] 2xl:w-[90vw] max-w-none px-2 sm:px-3 lg:px-4">
        ...
        <MarketSummary
          ...
          cells={[
            {
              label: "Total PV Sales",
              value: latestPV ? `${formatNumber(latestPV)} units` : null,
            },
            {
              label: "PV Growth Rate",
            ...
            {
              label: "Leading OEM",
              value: formatLeadingOemLabel(marketComputed?.chartData ?? []),
              tone: "primary",
            },
          ]}
        />
