  ...
  useEffect(() => {
    ...
    async function loadOemData() {
      try {
        ...
        console.error(err);
        if (!cancelled) {
          setOemError("Failed to load tractor OEM segment share data");
          setOemRaw([]);
        }
      } finally {
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        ...
        if (!cancelled) setGraphId(cfg?.tractor ?? null);
      } catch (err) {
  ...
  () =>
    buildLeadershipGrowthSummary({
      ...
      emptyMessage: "No tractor OEM segment share data available for the selected period.",
...
useEffect(() => {
  ...
  >
    {oemError ? (
      ...
      <div className="h-[350px] flex items-center justify-center text-sm text-muted-foreground">
        Loading tractor OEM segment share…
      </div>
    ) : oemChartData.length ? (
  ...
  Note: Includes diesel, electric (EV), and other alternative-fuel vehicles.
          ...
          <BrandModelsChart segmentName="tractor" title="Tractor Brand Models" />
