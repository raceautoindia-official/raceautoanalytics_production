  ...
  useEffect(() => {
    ...
    async function loadOemData() {
      try {
        ...
        console.error(err);
        if (!cancelled) {
          setOemError("Failed to load bus OEM segment share data");
          setOemRaw([]);
        }
      } finally {
        if (!cancelled) {
...
useEffect(() => {
  ...
  () =>
    buildLeadershipGrowthSummary({
      ...
      emptyMessage: "No bus OEM segment share data available for the selected period.",
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        ...
        if (!cancelled) setGraphId(cfg?.bus ?? null);
      } catch (err) {
  ...
  const segmentDonutData = useMemo(() => {
    ...
    // Only the selected month. No fallback: if it is not published the donut
    // renders nothing and its section hides.
    const picked = sorted.find((r) => r.month === wantedLabel);
    if (!picked) return [];

    const arr = [
      { name: "LCV", fullName: "LCV – Light Commercial Vehicle", value: Number(picked.lcv ?? 0) || 0, color: "#22C55E" },
      { name: "MCV", fullName: "MCV – Medium Commercial Vehicle", value: Number(picked.mcv ?? 0) || 0, color: "#0EA5E9" },
      {
        name: "HCV + Others",
        fullName: "HCV – Heavy Commercial Vehicle (and others)",
  ...
  const growthSummary = formatGrowthWithYoY(
    latestBus,
    prevBus,
    pickSeries({ data: prevYearBaseData } as any, ["Bus", "bus"]) ?? pickSeries(prevYearPoint, ["Bus", "bus"]),
  );
...
const showApplicationChartSection =
  ...
  leadingApp && appTotal
    ? (() => {
        ...
        return `${leadingApp.name} Application dominates at ${shareText}% share, with other applications supporting public and school transport demand.`;
      })()
    : appError
      ? appError
