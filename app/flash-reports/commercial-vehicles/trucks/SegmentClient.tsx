  ...
  useEffect(() => {
    ...
    async function loadOemData() {
      try {
        ...
        console.error(err);
        if (!cancelled) {
          setOemError("Failed to load truck OEM segment share data");
          setOemRaw([]);
        }
      } finally {
        if (!cancelled) {
...
useEffect(() => {
  ...
  () =>
    buildLeadershipGrowthSummary({
      ...
      emptyMessage: "No truck OEM segment share data available for the selected period.",
...
const showApplicationChartSection =
    ...
    <div className="min-h-screen py-0">
      <div className="mx-auto w-[95vw] xl:w-[93vw] 2xl:w-[90vw] max-w-none px-2 sm:px-3 lg:px-4">
        ...
        <MarketSummary
          ...
          cells={[
            {
              label: "Total Truck Sales",
              value: latestTruck ? `${formatNumber(latestTruck)} units` : null,
            },
            {
              label: "Truck Growth Rate",
  ...
  >
    {oemError ? (
      ...
      <div className="h-[350px] flex items-center justify-center text-sm text-muted-foreground">
        Loading truck OEM segment share…
      </div>
    ) : oemChartData.length ? (
  ...
  Note: Includes diesel, CNG, electric (EV), and other alternative-fuel vehicles.
          ...
          <BrandModelsChart segmentName="truck" title="Truck Brand Models" />
