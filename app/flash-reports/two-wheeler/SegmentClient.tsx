...
const res = await fetch(url);
  ...
  const oemComputed = useMemo(() => {
    ...
    // Merge any "Others"-like rows (handles admin-renamed labels such as
    // "Others[tata, mahindra, etc]") into a single bucket pinned to the end.
    // Prevents the duplicate-Others bar bug when admin updates the Others
    // label mid-quarter and historical/current month data carry different
    // exact strings for the same logical bucket.
...
useEffect(() => {
  ...
  // Mirrors the main flash-reports page pattern: config fetched once with the
  // initial region (always "india"), so graphId stays stable across country
  // changes. The forecast hook handles country-specific data via its own
  // country param — we should NOT re-fetch the graph config per country, since
  // doing so flips graphId mid-transition and breaks the forecast stack.
  useEffect(() => {
    let cancelled = false;
...
const appSummary = useMemo(() => {
  if (!appBarData.length) {
    return "This segment data will be available soon. For early access, contact info@raceautoanalytics.com.";
  }
    ...
    <div className="min-h-screen py-0">
      <div className="mx-auto w-[95vw] xl:w-[93vw] 2xl:w-[90vw] max-w-none px-2 sm:px-3 lg:px-4">
        ...
        <MarketSummary
          ...
          cells={[
            {
              label: "Total 2W Sales",
              value: latest2W ? `${formatNumber(latest2W)} units` : null,
            },
            {
              label: "2W Growth Rate",
