...
export async function POST(req: Request) {
  try {
    ...
    const [rows] = await db.execute(
      `SELECT id, mobile_number, verification_mode, email_verified, phone_verified,
              otp_last_sent_at
       FROM users
       WHERE email = ?
       LIMIT 1`,
    ...
    if (user.phone_verified) {
      return NextResponse.json(
        { message: "Phone already verified", pending: getPendingVerificationSteps(user) },
        { status: 200 },
      );
    }
    ...
    if (!otpResult.ok) {
      return NextResponse.json(
        {
          message: otpResult.error || "Unable to send OTP right now. Please try again shortly.",
        },
        { status: 502 },
      );
    }
