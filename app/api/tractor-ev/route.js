  ...
  try {
    ...
    for (const monthKey of allMonths) {
      ...
      const [cols] = await db.execute(
        `SHOW COLUMNS FROM tractor_ev LIKE '${safeColumn}'`
      );
      if (cols.length === 0) {
        await db.execute(
          `ALTER TABLE tractor_ev ADD COLUMN ${escapeColumn(
            monthKey
          )} FLOAT DEFAULT 0`
        );
      }
    ...
    for (const { company_name, status, months } of companyEntries) {
      ...
      const insertQuery = `
        INSERT INTO tractor_ev (company_name, status, ${monthColumns.join(
          ", "
        )})
