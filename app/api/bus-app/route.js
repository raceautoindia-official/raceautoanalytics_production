    ...
    for (const { application_segments, status, parsedMonths } of parsedRows) {
      ...
      for (const monthKey of Object.keys(parsedMonths)) {
        ...
        const [cols] = await db.execute(
          `SHOW COLUMNS FROM bus_app LIKE '${safeColumn}'`
        );
        if (cols.length === 0) {
          await db.execute(
            `ALTER TABLE bus_app ADD COLUMN ${escapeColumn(
              monthKey
            )} FLOAT DEFAULT 0`
          );
        }
      ...
      const insertQuery = `
        INSERT INTO bus_app (application_segments, status, ${monthColumns.join(
          ", "
        )})
