    ...
    for (const { application_segments, status, parsedMonths } of parsedRows) {
      ...
      for (const monthKey of Object.keys(parsedMonths)) {
        ...
        const [cols] = await db.execute(
          `SHOW COLUMNS FROM passenger_app LIKE '${safeColumn}'`
        );
        if (cols.length === 0) {
          await db.execute(
            `ALTER TABLE passenger_app ADD COLUMN ${escapeColumn(
              monthKey
            )} FLOAT DEFAULT 0`
          );
        }
      ...
      const insertQuery = `
        INSERT INTO passenger_app (application_segments, status, ${monthColumns.join(
          ", "
        )})
