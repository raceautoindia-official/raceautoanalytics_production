...
export async function GET() {
  try {
    const tableName = "tractor_overall";
  ...
  try {
    ...
    for (const monthKey of allMonths) {
      ...
      const [cols] = await db.execute(
        `SHOW COLUMNS FROM tractor_overall LIKE '${safeColumn}'`
      );
      if (cols.length === 0) {
        await db.execute(
          `ALTER TABLE tractor_overall ADD COLUMN ${escapeColumn(
            monthKey
          )} FLOAT DEFAULT 0`
        );
      }
    ...
    for (const { company_name, status, months } of companyEntries) {
      ...
      const insertQuery = `
        INSERT INTO tractor_overall (company_name, status, ${monthColumns.join(
          ", "
        )})
